...
3. **대시보드 서브 탭 (확인 필요 / 기록 상세)**:
   - **[⚠️ 확인 필요] 탭**:
     ...
     - **입실/퇴실 시간대별 서브 탭 필터 (`alert-filter-tabs`)**: 등록된 학생들의 입실/퇴실 기준시간(예: `09:00`, `10:00`, `13:00` 등)을 자동 집계하여 시간대별 칩 버튼을 생성. `[전체]` 칩 없이 개별 시간대를 자유롭게 다중 선택(Multi-Select Toggle) 가능하며, 아무것도 선택하지 않으면 전체 학생을 기본 표시. 모바일(450px 이하)에서는 타이틀/화살표와 탭 바가 2단으로 정갈하게 분리되어 100% 가로 폭으로 쾌적하게 스와이프 탐색 가능.
